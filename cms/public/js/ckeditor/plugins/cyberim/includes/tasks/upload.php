			...
			case UPLOAD_ERR_OK :
				if (!preg_match(',[[:cntrl:]]|[/\\:\*\?\"\<\>\|],', $_FILES['file']['name'])){
					...
					if (in_array(strtolower($ext), $fileTypes)) {
						...
						if (Manager::$conf['resize_upload']['max_width'] && Manager::$conf['resize_upload']['max_height'] && strtolower($ext)!='png' && strtolower($ext)!='gif') {
							tools_photo::create_thumb(
									Manager::$conf['resize_upload']['max_width'],
									Manager::$conf['resize_upload']['max_height'], 
									FileManager::convertToFileSystem($targetFile), 
									FileManager::convertToFileSystem($targetFile), 
									80
							);
						}
